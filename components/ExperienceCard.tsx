...
const ExperienceCard: React.FC<ExperienceCardProps> = ({ experience }) => {
  ...
  return (
    <div
      ref={ref}
      className={`bg-white/80 dark:bg-gray-800/80 p-4 sm:p-6 md:p-8 rounded-xl border border-gray-200/50 dark:border-gray-700/50 shadow-sm hover:shadow-xl hover:scale-[1.02] transition-all backdrop-blur-sm scroll-slide-left group ${isVisible ? "visible" : ""}`}
    >
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2">
        <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-all">
          {experience.title}
        </h3>
        ...
        <div className="flex items-center">
          {experience.logo && (
            <img
              ...
              loading="lazy"
              decoding="async"
              className="w-8 h-8 sm:w-9 sm:h-9 mr-3 object-contain rounded-md"
            />
          )}
